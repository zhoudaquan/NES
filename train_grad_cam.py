...
def main():
    ...
    output_dir = ''
    layer_num = '13'
    grad_cam = GradCam(model=model.cpu(),\
            target_layer_names=[layer_num], use_cuda=False)
    img = cv2.imread('pytorch-grad-cam/examples/both.png', 1)
    ...
    gb_model = GuidedBackpropReLUModel(model=model.cpu(), use_cuda=False)
    gb = gb_model(input, index=target_index)
    gb = gb.transpose((1, 2, 0))
    cam_mask = cv2.merge([mask, mask, mask])
    cam_gb = deprocess_image(cam_mask*gb)
    gb = deprocess_image(gb)
    cv2.imwrite(layer_num + '/gb.jpg', gb)
    cv2.imwrite(layer_num + '/cam_gb.jpg', cam_gb)
    import pdb; pdb.set_trace()

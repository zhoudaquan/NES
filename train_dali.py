...
# use nvidia-dali for fast training
class HybridTrainPipe(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop, num_gpus, dali_cpu=False):
        super(HybridTrainPipe, self).__init__(batch_size, num_threads, device_id, seed=12 + device_id)
        self.input = ops.MXNetReader(path = [data_dir+"/train.rec.rec"], 
        index_path=[data_dir+"/train.rec.idx"], random_shuffle = True, shard_id = device_id, num_shards = num_gpus)
...
class HybridValPipe(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop, size, num_gpus, dali_cpu=False):
        ...
        self.input = ops.MXNetReader(path = [data_dir+"/val.rec.rec"], index_path=[data_dir+"/val.rec.idx"],
...
def main():
    ...
    pipes = [HybridTrainPipe(batch_size=int(args.batch_size/args.num_gpu), num_threads=args.workers, device_id=device_id, data_dir=args.data, crop=224, num_gpus=args.num_gpu, dali_cpu=False) for device_id in range(args.num_gpu)]
    pipes[0].build()
    loader_train = plugin_pytorch.DALIClassificationIterator(pipes, size=int(pipes[0].epoch_size("Reader")))
    ...
    # )
    # 
    pipes = [HybridValPipe(batch_size=int(100/args.num_gpu), num_threads=4, device_id=device_id, data_dir=args.data, crop=224, size=256, num_gpus=args.num_gpu, dali_cpu=False) for device_id in range(args.num_gpu)]
    pipes[0].build()
    loader_eval = plugin_pytorch.DALIClassificationIterator(pipes, size=int(pipes[0].epoch_size("Reader")))
...
def train_epoch(
    ...
    last_idx = loader._size
    # num_updates = epoch * len(loader)
    num_updates = epoch * loader._size
    #for batch_idx, (input, target) in enumerate(loader):
    for batch_idx, data in enumerate(loader):
        last_batch = batch_idx == last_idx
        data_time_m.update(time.time() - end)
        # import pdb; pdb.set_trace()
        input = torch.cat([data[0]['data'] for j in range(args.num_gpu)], dim=0)
        target = torch.cat([data[0]['label'] for j in range(args.num_gpu)], dim=0).cuda().long()

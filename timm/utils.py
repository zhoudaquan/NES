...
class CheckpointSaver:
    ...
    def save_checkpoint(self, model, optimizer, args, epoch, model_ema=None, metric=None):
        ...
        if (len(self.checkpoint_files) < self.max_history
            ...
            # NOTE uncomment the following line if want to save all checkpoints
            # filename = '-'.join([self.save_prefix, str(epoch)]) + self.extension
            filename = '-'.join([self.save_prefix, "current"]) + self.extension
...
class ModelEma:
    ...
    def update_ema(self, model, decay=0.9999, device='', resume=''):
        # update the ema history model when architecture is modified
        self.ema = deepcopy(model)
        self.ema.eval()
        self.decay = decay
        self.device = device  # perform ema on different device from model if set

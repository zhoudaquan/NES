...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params, enable_se=True, group_se = True, sampling = True, sampling_rate = 0.5, half_up_sampling=1, 
        group_first_1x1 =False, rm_first_1x1=False, shortcut_coeff=False, mixnet=True):
        super().__init__()
        ...
        if mixnet:
            group1 = oup//2
            group2 = int(oup - oup//2)
            self._depthwise_conv_7x7 = Conv2dSamePadding(
            in_channels=group1, out_channels=group1, groups=group1,  # groups makes it depthwise
            kernel_size=k, stride=s, bias=False)
            self._bn1_7x7 = nn.BatchNorm2d(num_features=group1, momentum=self._bn_mom, eps=self._bn_eps)
            
            self._depthwise_conv = Conv2dSamePadding(
            in_channels=group2, out_channels=group2, groups=group2,  # groups makes it depthwise
            kernel_size=k, stride=s, bias=False)
            self._bn1 = nn.BatchNorm2d(num_features=group2, momentum=self._bn_mom, eps=self._bn_eps)
        else:
            self._depthwise_conv = Conv2dSamePadding(
    ...
    def forward(self, inputs, drop_connect_rate=None):
        ...
        if self.mixnet:
            ...
            x1 = relu_fn(self._bn1(self._depthwise_conv(x[:,:split,:,:])))
            x2 = relu_fn(self._bn1_7x7(self._depthwise_conv_7x7(x[:,split:,:,:])))

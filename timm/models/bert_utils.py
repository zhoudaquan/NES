...
class WSConv2d_bert(nn.Conv2d):
    def __init__(self, in_planes, out_channels,
                 kernel_size, stride=1, padding=0, dilation=1,
                 groups=1, bias=True, jump = 2.0, rep_dim = 1, repeat_weight = True, use_coeff=False, mgroup = 4):
        if rep_dim == 0:
            # this is repeat along the filter dim (dimension 0 of the weights tensor)
            base_len = out_channels // mgroup
            channel_len = base_len + (mgroup-1) * jump
            super(WSConv2d_bert, self).__init__(
                int(in_planes), int(np.ceil(channel_len)),
                kernel_size, stride=stride, padding=padding, dilation=dilation,
                groups=groups, bias=bias)
            self.rep_time = int(np.ceil(
            1. * out_channels / self.weight.shape[0]))
        elif rep_dim == 1:
            # this is to repeat along the channel dim(dimension 1 of the weights tensor)
            base_len = in_planes // mgroup
            channel_len = base_len + (mgroup-1) * jump
            super(WSConv2d_bert, self).__init__(
                int(np.ceil(channel_len)),int(out_channels), 
                kernel_size, stride=stride, padding=padding, dilation=dilation,
                groups=groups, bias=bias)
            self.rep_time = int(np.ceil(
            1. * in_planes / self.weight.shape[1]))
        ...
        self.reuse = False
        self.coeff_grad = None
    def gen_weight(self,dim):
        new_weight = []
        if dim == 0:
            for i in range(self.mgroup):
                start_idx = int(i * self.jump)
                new_weight.append(self.weight[start_idx:int(start_idx + self.base_len), :, :, :])
        elif dim == 1:
            for i in range(self.mgroup):
                start_idx = int(i * self.jump)
                new_weight.append(self.weight[:, start_idx:int(start_idx + self.base_len), :, :])
        return torch.cat(new_weight,dim=dim)
    def forward(self, x):
        x.cpu()
        ih, iw = x.size()[-2:]
        kh, kw = self.weight.size()[-2:]
        ...
        # print("use_coeff == False")
        if self.rep_dim == 0:
            out = F.conv2d(x, self.gen_weight(dim=0)[:self.out_channels_ori,:,:,:], None, 1)
        else:
            out = F.conv2d(x, self.gen_weight(dim=1)[:,:x.shape[1],:,:], None, 1)
        return out

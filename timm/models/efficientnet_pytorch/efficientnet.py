...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params, enable_se=True, group_se = True, sampling = True, sampling_rate = 0.5, half_up_sampling=1, 
        group_first_1x1 =False, rm_first_1x1=False, shortcut_coeff=False):
        super().__init__()
        ...
        if self._block_args.expand_ratio != 1:
            try:
                if self.rm_first_1x1:
                    ...
                    x = relu_fn(self._bn0_1(relu_fn(self._bn0(self._expand_conv(inputs)))))
            except:
                import pdb;pdb.set_trace()
        x = relu_fn(self._bn1_1(relu_fn(self._bn1(self._depthwise_conv(x)))))
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, margs = None):
        ...
        if margs is None:
            ...
            self.half_up_sampling = margs.half_up_sampling

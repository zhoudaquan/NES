...
class WSConv2d_v1(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        if self.rep_dim == 0:
            ...
            # use following to save actual computation time
            out = F.conv2d(x, self.weight, None, 1).repeat([1,self.rep_time,1,1])[:,:self.out_channels_ori,:,:]
        else:
            # out = F.conv2d(x, self.weight.repeat([1,self.rep_time,1,1])[:,:x.shape[1],:,:], None, 1)
            # import pdb; pdb.set_trace();
            # use following to save the actual inference time
            out = F.conv2d(x[:,:x.shape[1]//2,:,:] + x[:,x.shape[1]//2:,:,:], self.weight, None, 1)
        return out

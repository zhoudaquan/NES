...
from brevitas.nn.quant_activation import QuantReLU, QuantActivation
...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params, enable_se=True, group_se = True, sampling = True, sampling_rate = 0.5, half_up_sampling=1, 
        group_first_1x1 =False, rm_first_1x1=False, shortcut_coeff=False, quant = False, quant_act = False):
        super().__init__()
        ...
        if quant_act:
            self.quant_relu1 = QuantReLU(quant_type=QuantType.INT, bit_width=8, max_val=6)
            self.quant_relu2 = QuantReLU(quant_type=QuantType.INT, bit_width=8, max_val=6)
            ...
            if group_first_1x1:
                ...
                if not sampling:
                    if quant:
                        ...
                        #self._expand_conv = qnn.QuantConv2d(in_channels=inp, out_channels=oup, kernel_size=1, bias=False,
                        #weight_quant_type=QuantType.INT, weight_bit_width=8, padding_type=PaddingType.SAME)
                    else:
    ...
    def forward(self, inputs, drop_connect_rate=None):
        ...
        if self._block_args.expand_ratio != 1:
            try:
                if self.rm_first_1x1:
                    # if self.quant_act:
                    if False:
                        x = self.quant_relu1(self._bn0(self.sampling_feature(x, self.oup)))
                    else:
        ...
        # if self.quant_act:
        if False:
            x = self.quant_relu1(self._bn1(self._depthwise_conv(x)))
        else:

...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params, enable_se=True, group_se = True, sampling = True, sampling_rate = 0.5, half_up_sampling=1, 
        ...
        if self._block_args.expand_ratio != 1 and (not rm_first_1x1):
            if group_first_1x1:
                ...
                if not sampling:
                    ...
                    self._expand_conv = NESConv2d(inp, oup, kernel_size=1, bias=False, multiplier = multiplier1, rep_dim=0, use_coeff=False)
        ...
        if not sampling:
            ...
            self._project_conv = NESConv2d(oup, final_oup, kernel_size=1, bias=False, multiplier = multiplier2, rep_dim=1, use_coeff=False)
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, margs = None):
        ...
        if sampling:
            self._conv_head = NESConv2d(in_channels, out_channels, kernel_size=1, bias=False, multiplier = 4/2, rep_dim=1, use_coeff=False)
        else:
